...
func New(ctx context.Context, config ReplConfig, applications AppMap, extensionControls []core.ExtensionControl) TerminusModel {
...
func (m TerminusModel) Init() tea.Cmd {
	return tea.Tick(time.Millisecond*500, func(t time.Time) tea.Msg {
		return tickMsg(t)
	})
}

func (m TerminusModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case tea.KeyEnter:
			...
			if command != "" {
				...
				output, err := m.session.vm.ExecuteCommand(command)
				if err != nil {
				...
				return m, func() tea.Msg {
					return commandOutputMsg{output: output, isErr: false}
				}
			}
			return m, nil
...
func (m TerminusModel) View() string {
	if m.quitting {
		return "Goodbye!\n"
	}
...
func (m *TerminusModel) LaunchApp(app App, args []string) tea.Cmd {
	m.buffer = ""
	m.cursor = 0
	m.currentApp = app
