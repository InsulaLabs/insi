...
# Script to test the atomic TKV operations (setnx, cas) of the insic CLI
...
expect_conflict() {
    ...
    if [ "$exit_code" -ne 0 ]; then
        if echo "$output_content" | grep -q "Conflict"; then
            echo -e "${SUCCESS_EMOJI} ${GREEN}SUCCESS (Received Expected Conflict): ${description}${NC}"
            SUCCESSFUL_TESTS_COUNT=$((SUCCESSFUL_TESTS_COUNT + 1))
        else
            echo -e "${FAILURE_EMOJI} ${RED}FAILURE: ${description} - Expected a Conflict error, but got a different error (Exit code: $exit_code)${NC}"
            echo -e "   Output: ${output_content}"
            FAILED_TESTS_COUNT=$((FAILED_TESTS_COUNT + 1))
        fi
    else
        echo -e "${FAILURE_EMOJI} ${RED}FAILURE: ${description} - Expected a Conflict error, but command succeeded (Exit code: $exit_code)${NC}"
        FAILED_TESTS_COUNT=$((FAILED_TESTS_COUNT + 1))
    fi
}
...
test_setnx() {
    print_header "Test: setnx"
    local key="test_setnx_$(date +%s)"
    local value1="value1"
    local value2="value2"
    ...
    exit_code1=$?
    expect_success "setnx on new key '$key'" "$exit_code1" "$output1"
    ...
    exit_code2=$?
    expect_conflict "setnx on existing key '$key'" "$exit_code2" "$output2"
...
test_cas() {
    print_header "Test: cas (Compare-And-Swap)"
    local key="test_cas_$(date +%s)"
    ...
    sleep 1 # Allow a moment for the write to be fully applied to the state machine - this sleep is for non-atomic and the client may hit a different node so need network sync first
...
main() {
    ...
    if [ "$ping_exit_code" -ne 0 ]; then
        ...
        echo -e "${SUCCESS_EMOJI} ${GREEN}Server is online. Proceeding with atomic op tests.${NC}"
    fi
    ...
    if [ "$FAILED_TESTS_COUNT" -gt 0 ]; then
        ...
        echo -e "${GREEN}All atomic op tests passed successfully!${NC}"
    fi
}
